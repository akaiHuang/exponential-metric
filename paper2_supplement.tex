...
We present three consistent physical interpretations of the gravitational
entropy production $\Sgrav = -\ln(-g_{00})$, showing that all three
yield the same quantity from different physical settings.
For the exponential metric, $-g_{00} = e^{-\rs/r}$, all
...
All three frameworks take the metric $g_{00}$ as input and produce the
same information-theoretic quantity $\Sgrav$.  This convergence is a
consistency check confirming that the identification
$\Sgrav = -\ln(-g_{00})$ is robust across different physical settings,
not three independent derivations of $\Sgrav$ from first principles.
...
Following the results of Dorau and Much~\cite{DorauMuch2025},
who derived Einstein's equations from quantum relative entropy,
this route applies their modular Hamiltonian framework to extract
$\Sgrav$ from the algebraic quantum field theory
(AQFT) setting using the Bisognano--Wichmann theorem and the
entanglement first law.  The derivation below is theirs;
our contribution is the identification of the result with the
Petz recovery fidelity bound (Step~5).
