...
fidelity.
The exponential metric $g_{00} = -e^{-\rs/r}$ corresponds to
the \emph{lower bound geometry}---the metric where recovery fidelity
equals the JRSWW bound $F = e^{-\Sgrav/2}$.  At a Schwarzschild event
horizon, $\tauasym \to 1$ (complete information loss), while in the
exponential metric no horizon forms.
We show that Pikovski's gravitational decoherence mechanism is
naturally a Petz recovery problem, and that several results
...
The central result of this paper is that
Eq.~\eqref{eq:ceff} is \emph{identical in form} to the Petz
recovery fidelity bound~\eqref{eq:petz_bound}.
As shown in the Supplemental Material (Theorem~1), the
gravitational redshift defines a pure-loss bosonic channel
with transmissivity $\eta = -g_{00}$, whose unique extensive
(additive under composition) entropy production is
$\Sgrav = -\ln(-g_{00}) = \rs/r$ for the exponential metric.
This is a \emph{theorem}---not a definition---following from
the extensivity requirement and the standard GR identification
$\eta = -g_{00}$.  Writing $\Sgrav = 2|\Phi|/c^2 = \rs/r$
(where $\rs = 2GM/c^2$), we obtain:
\begin{equation}
  \boxed{\;
  \frac{\ceff}{c}
  \;=\; e^{-\Sgrav/2}
...
bound---where the JRSWW bound defines the geometry.
This identification means the exponential metric is the
\emph{lower bound geometry}: by Theorem~1
in the Supplemental Material, exact saturation is impossible
for $\Sgrav > 0$, so the actual recovery fidelity strictly
exceeds $e^{-\Sgrav/2}$.  The exponential metric is the
geometry where $F$ equals this lower bound, and any deviation
from it is an empirical question~[IDENTIFICATION, not hypothesis].
...
We emphasize from the outset what this identification does \emph{not}
provide: it does not predict dark matter abundances, dark energy
values, or allow reconstruction of the metric from $\tauasym$ alone
(see Sec.~\ref{sec:scope} for a complete accounting).
...
or the exponential metric (bound geometry) is an empirical
...
\begin{proposition}[Characterization of the exponential metric]
Among static, spherically symmetric metrics $g_{00} = -f(r)$,
the exponential metric $f(r) = e^{-\rs/r}$ is the unique solution
for which $\Sgrav \equiv -\ln(-g_{00}) = \rs/r$ is exactly linear
in $\rs/r$.  The Schwarzschild metric gives
$\Sgrav^{\mathrm{Schw}} = -\ln(1 - \rs/r) = \rs/r + \tfrac{1}{2}(\rs/r)^2 + \cdots$,
which is linear only to leading order.
\end{proposition}

We do not derive this metric from an action principle; rather,
we identify it as the metric that exactly linearizes the
QRE--gravity correspondence~\eqref{eq:main_identification}.
...
We observe that this mechanism has the \emph{formal structure} of a
Petz recovery problem, though it is fundamentally distinct from the
gravitational channel $\Sgrav$.
...
(Three key distinctions between the Pikovski channel and $\Sgrav$ are
detailed in Supplement~S6: the Pikovski entropy is probe-dependent,
has no finite supremum, and dephases in the position basis rather than
the energy basis.)
